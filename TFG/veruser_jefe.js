...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 3) Cargar días del manager en el sidebar
  await cargarSidebarDias();

  // 4) Renderizar perfil del empleado
  renderPerfil(empleado, disponibilidad);
...
// ═══════════════════════════════════════════════════════
function renderPerfil(emp, disp) {
  ...
  const pendientes = disp?.dias_pendientes  || 0;

  document.getElementById('statTotal').textContent      = total;
  document.getElementById('statConsumidos').textContent = consumidos;
  document.getElementById('statPendientes').textContent = pendientes;

  // Historial → por ahora vacío (pendiente endpoint /solicitudes del empleado)
  renderHistorial([]);

  // Próximo descanso → por ahora vacío
  renderProximoDescanso(null);
}

// ═══════════════════════════════════════════════════════
//   HISTORIAL DE SOLICITUDES
//   ⚠️ Pendiente: GET /solicitudes?usuario_id=X
// ═══════════════════════════════════════════════════════
  ...
  tbody.innerHTML = solicitudes.map(s => {
    ...
    return `
      <tr>
        ...
        <td class="tipo-name">${s.tipo || '—'}</td>
        <td><span class="status-tag ${claseEstado}">● ${(s.estado || '').toUpperCase()}</span></td>
        <td><button class="btn-accion" title="Opciones">•••</button></td>
      </tr>
    `;
  }).join('');
}
