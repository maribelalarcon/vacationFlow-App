...
// ─── OCULTAR PANEL JEFE SI NO ES MANAGER ─────────────────
...
async function cargarPendientes() {
  try {
    ...
    const ficticios = [
      { id: 1, empleado: { nombre: 'Elena', apellidos: 'Rivas' }, tipo: 'vacaciones', fecha_inicio: '2024-07-12', fecha_fin: '2024-07-29', total_dias: 8 },
      { id: 2, empleado: { nombre: 'Marc', apellidos: 'Soler'  }, tipo: 'baja_medica', fecha_inicio: '2024-08-05', fecha_fin: '2024-08-12', total_dias: 7 },
    ];
    renderSolicitudes(ficticios);
    actualizarBadge(ficticios.length);
    actualizarHistorico(ficticios.length);
  }
}
  ...
  solicitudes.forEach((s, i) => {
    ...
    card.innerHTML = `
      <div class="sol-top">
        ...
        <div>
          <div class="sol-nombre">${s.empleado.nombre} ${s.empleado.apellidos}</div>
...
async function aprobar(btn, id) {
  try {
    ...
    // Sin conexión: efecto visual igualmente
    btn.closest('.solicitud-card').classList.add('aprobada');
    btn.disabled = true;
    btn.closest('.solicitud-card').querySelector('.btn-rechazar').disabled = true;
  ...
  for (let i = 0; i < totalCells; i++) {
    ...
    eventos.forEach(ev => {
      ...
      if (cellDate >= ini && cellDate <= fin) {
        ...
        bar.textContent = cellDate.getTime() === ini.getTime() ? ev.nombre : '';
        bar.title = ev.nombre;
...
async function cargarCalendario() {
  ...
  try {
    const res = await fetch(
      `${BASE}/admin/calendar?anio=${currentYear}&mes=${currentMonth + 1}`,
